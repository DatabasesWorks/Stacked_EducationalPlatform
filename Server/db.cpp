...
std::string DB::authenticate(MYSQL* connection, std::string payload){
    ...
    QString query = "SELECT * FROM users WHERE username=\"" +split.front()+"\" AND password=\"" +split.back()+"\";";
...
std::string DB::regUser(MYSQL* connection, std::string payload){
    ...
    //if student and class not present error
         if((mysql_num_rows(result) != 0 && split.at(3)="1") || (mysql_num_rows(result) == 0 && split.at(3)=="0")){
             return "CLASSERROR";
         }
